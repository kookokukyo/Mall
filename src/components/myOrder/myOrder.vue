<template>
  <div class="myorder">
    <Header :title="'我的订单'" :noback="true" :right="false"></Header>
    <div class="top">
      ...
      <div class="list">
        <ul>
          <li @click="cut(0)" :class="option==0?'show':''">全部</li>
          <li @click="cut(1)" :class="option==1?'show':''">待付款</li>
          <li @click="cut(2)" :class="option==2?'show':''">待上传</li>
          <li @click="cut(3)" :class="option==3?'show':''">待审核</li>
          <li @click="cut(4)" :class="option==4?'show':''">待生产</li>
          <li @click="cut(5)" :class="option==5?'show':''">待发货</li>
          <li @click="cut(6)" :class="option==6?'show':''">待收货</li>
        </ul>
        <div class="clear"></div>
      </div>
    </div>
      ...
      <div class="mid_list">
        <div class="mid_item" v-show="!option || option == item.state" v-for="(item,index) in orderlist">
          ...
          <div class="btns">
            ...
            <button v-show="item.state==1" @click="$router.push('/confirmOrder')">确认订单</button>
            <button v-show="item.state==2" @click="$router.push('/uploadContract')">上传合同</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  activated() {
    if (this.option != this.$route.query.option && this.midscroll) {
      this.midscroll.scrollTo(0, 0, 0);
    }
    this.option = this.$route.query.option ? this.$route.query.option : 0;
  }
};
</script>
...
.mid {
  margin-top: cp(184);
  height: calc(100vh - 40px - 50px - 42px);
  overflow: hidden;
}
